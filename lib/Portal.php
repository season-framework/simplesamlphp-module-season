...
namespace SimpleSAML\Module\season;
...
class Portal
{
    ...
    public function getMenu($thispage)
    {
        ...
        foreach ($this->pages as $pageid => $page) {
            ...
            if (!isset($page['href'])) {
                $text .= '<li class="nav-item active"><a class="nav-link" href="#">' . $name . '</a></li>';
            } elseif ($pageid === $thispage) {
                $text .= '<li class="nav-item active"><a class="nav-link" href="#">' . $name . '</a></li>';
            } else {
                $text .= '<li class="nav-item"><a class="nav-link" href="' . $page['href'] . '">' . $name . '</a></li>';
            }
        }
        $text .= '</ul>';
